...
def progresso_hook(d):
    if d['status'] == 'downloading':
        ...
        if total > 0:
            ...
            vel_kb = velocidade / 1024 if velocidade else 0
            sys.stdout.write(f"\r  -> Baixando: {baixado_mb:.2f} MB / {total_mb:.2f} MB ({porcentagem:.1f}%) a {vel_kb:.1f} KB/s")
            sys.stdout.flush()
    elif d['status'] == 'finished':
        tam_mb = (d.get('total_bytes', 0)) / (1024 * 1024)
        print(f"\n  -> Download concluido! Tamanho final: {tam_mb:.2f} MB. Processando video...")

def obter_ydl_opts(nome_saida):
...
PERFIL_ALVO = "goldfitnesssl.centro"
LIMITE_POSTS = 15
